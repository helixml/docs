...
This page describes the environment variables used to configure Helix.
...
- `RUNTIME_AXOLOTL_WARMUP_MODELS`: Comma-separated list of Mistral-7B and SDXL models to pre-warm
- `RUNTIME_OLLAMA_ENABLED`: Enable/disable LLM inference with Ollama backend (default `true`) 
- `RUNTIME_OLLAMA_WARMUP_MODELS`: Comma-separated list of LLM models to enable and pre-download for inference

### Important notes

- The LLM models specified in `RUNTIME_OLLAMA_WARMUP_MODELS` must be included in the Helix Docker image you are using. See available models per release: https://github.com/helixml/helix/releases
- If using Helix on Kubernetes, the Helm chart values like `runner.models` map to the `RUNTIME_OLLAMA_WARMUP_MODELS` env var.

The full list of available environment variables can be found in `config.go` and `runner_config.go`.
