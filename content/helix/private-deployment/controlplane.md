...
* Helix will download the weights for models specified in `RUNTIME_OLLAMA_WARMUP_MODELS` at startup if they are not baked into the image. This can be slow, especially if it runs in parallel across many runners, and can easily saturate your network connection. This is why using the pre-baked weights is recommended.
